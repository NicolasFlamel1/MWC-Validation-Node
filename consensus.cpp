...
// Check if floonet
	...
	const Header Consensus::GENESIS_BLOCK_HEADER(
		...
		// Previous block hash
		(const uint8_t []){0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00},
		
		// Previous header root
		(const uint8_t []){0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x25, 0x76, 0x47, 0xFB, 0x29, 0xCE, 0x96, 0x4D, 0xDF, 0x2B, 0x27, 0xC6, 0x39, 0xAE, 0x60, 0xC4, 0xC9, 0x0F, 0xAF, 0xE5, 0xC4, 0x2E, 0x53},
		
		// Output root
		(const uint8_t []){0x69, 0x85, 0x96, 0x6E, 0xFC, 0xD7, 0x41, 0xC9, 0xEE, 0x42, 0xFE, 0x1A, 0x01, 0x64, 0x76, 0xB7, 0x4F, 0xFA, 0x1E, 0x81, 0x8D, 0xFD, 0x4D, 0x3B, 0x44, 0x1F, 0x8E, 0x0F, 0x87, 0x6A, 0xA2, 0x28},
		
		// Rangeproof root
		(const uint8_t []){0x43, 0xC1, 0xAC, 0x00, 0x25, 0xE1, 0x7F, 0xB6, 0x77, 0xB3, 0x32, 0xC6, 0x0E, 0x0D, 0x8B, 0x46, 0xCE, 0x87, 0x1D, 0xF8, 0x76, 0x49, 0x8C, 0x0D, 0xD5, 0xC9, 0x5E, 0x42, 0x94, 0xAE, 0x8F, 0xE5},
		
		// Kernel root
		(const uint8_t []){0x9B, 0xD2, 0xD3, 0x76, 0x44, 0x0E, 0x2B, 0x68, 0xEE, 0x1B, 0x06, 0x6B, 0xEF, 0xED, 0x59, 0x4B, 0x0A, 0x3D, 0x9F, 0xB5, 0x6D, 0x20, 0xAE, 0x04, 0x30, 0x59, 0xF5, 0xDB, 0x4D, 0x25, 0xD6, 0xE0},
		...
		// Proof nonces
		(const uint64_t []){3192500, 6825707, 24230992, 31245203, 53163694, 90654995, 106472612, 110444199, 139989294, 156335087, 156355985, 183386417, 189157284, 207907104, 213905482, 215326841, 220116398, 256066326, 259812081, 260939712, 272131888, 273570144, 282535412, 304151827, 322481271, 326494676, 355927801, 361940398, 369475836, 386602103, 399551873, 409685415, 416682585, 419304710, 435496048, 447341740, 462273908, 468790263, 491944474, 494233402, 511976431, 533915547}
	);
	...
	const Kernel Consensus::GENESIS_BLOCK_KERNEL(
		...
		// Excess
		(const uint8_t []){0x09, 0x3D, 0x0A, 0xEA, 0xE5, 0xF6, 0xAA, 0xB0, 0x97, 0x50, 0x96, 0xFD, 0xE3, 0x1E, 0x1A, 0x21, 0xFA, 0x42, 0xED, 0xFC, 0x93, 0xDB, 0x31, 0x8A, 0x10, 0x64, 0x15, 0x6A, 0xCE, 0x81, 0xF5, 0x46, 0x71},
		
		// Signature
		(const uint8_t []){0xCE, 0x1D, 0x97, 0xEF, 0x2F, 0x2C, 0xDB, 0x67, 0x64, 0xF0, 0x4C, 0x34, 0xE7, 0xAE, 0x95, 0x81, 0xED, 0xA4, 0xEA, 0x3C, 0xE8, 0x95, 0x5A, 0x5E, 0xA1, 0x5D, 0x83, 0x94, 0x78, 0x51, 0xA1, 0x9B, 0xAA, 0xB1, 0xFA, 0x40, 0x42, 0x19, 0x2C, 0x52, 0xA4, 0xE3, 0x96, 0x05, 0x0A, 0xA6, 0x34, 0x96, 0x16, 0xB3, 0x0F, 0x32, 0x51, 0x0F, 0x72, 0x09, 0x34, 0xEF, 0xEA, 0x50, 0x52, 0x76, 0x92, 0x1E},
	...
	const Output Consensus::GENESIS_BLOCK_OUTPUT(
		...
		// Commitment
		(const uint8_t []){0x09, 0x05, 0xA2, 0xEB, 0xF3, 0x91, 0x3C, 0x7D, 0x37, 0x86, 0x60, 0xA7, 0xB6, 0x0E, 0x6B, 0xDA, 0x98, 0x3B, 0xE4, 0x51, 0xCB, 0x1D, 0xE8, 0x77, 0x9A, 0xD0, 0xF5, 0x1F, 0x4D, 0x2F, 0xB0, 0x79, 0xEA},
...
// Otherwise
	...
	const Header Consensus::GENESIS_BLOCK_HEADER(
		...
		// Previous header root
		(const uint8_t []){0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x04, 0xF2, 0xFB, 0x2E, 0xE7, 0x49, 0x92, 0x3A, 0x81, 0x31, 0x02, 0x8A, 0xEA, 0xE6, 0x37, 0x07, 0x0B, 0x0B, 0x41, 0x45, 0x61, 0x7D, 0x42},
		
		// Output root
		(const uint8_t []){0x2D, 0x67, 0x2C, 0x22, 0x8F, 0x88, 0x36, 0x46, 0xD7, 0x5E, 0x73, 0xF1, 0xB9, 0x36, 0x99, 0x1F, 0xDF, 0xB1, 0x1B, 0xB1, 0x77, 0xEC, 0xA4, 0x51, 0x58, 0x7F, 0x97, 0xDE, 0xA1, 0xA4, 0xF9, 0x94},
		
		// Rangeproof root
		(const uint8_t []){0x34, 0x60, 0x12, 0x3E, 0x2E, 0x24, 0xC9, 0xDF, 0x65, 0xCD, 0x7A, 0x8D, 0x0E, 0x13, 0x84, 0x72, 0x4C, 0x23, 0xC8, 0x8C, 0x71, 0xB0, 0xCB, 0x32, 0xD8, 0x90, 0x85, 0x46, 0x48, 0xC7, 0x63, 0x7A},
		
		// Kernel root
		(const uint8_t []){0xCD, 0x3E, 0x07, 0xCF, 0xF2, 0xC0, 0xF8, 0xE4, 0x34, 0xC3, 0x0A, 0x47, 0xD6, 0x23, 0x12, 0x70, 0xD1, 0x2D, 0x42, 0x63, 0x8C, 0x0A, 0x7F, 0xD2, 0xC5, 0xC3, 0x12, 0x5D, 0x51, 0x92, 0x0D, 0x00},
	...
	const Kernel Consensus::GENESIS_BLOCK_KERNEL(
		...
		// Excess
		(const uint8_t []){0x08, 0xB6, 0x59, 0xFD, 0xE3, 0xA4, 0x12, 0x84, 0x81, 0x9F, 0x45, 0x41, 0x58, 0x90, 0x33, 0x02, 0x72, 0xEF, 0xEF, 0x7E, 0xF9, 0x91, 0xF8, 0x33, 0xA6, 0x4B, 0x74, 0x6B, 0xE8, 0x02, 0xC8, 0xFD, 0x77},
		
		// Signature
		(const uint8_t []){0xBD, 0x34, 0x3C, 0x89, 0xAC, 0xA0, 0x86, 0x45, 0x11, 0x2F, 0x52, 0x56, 0xA9, 0x88, 0x04, 0xF0, 0x68, 0xBC, 0x08, 0xB9, 0x5A, 0xAA, 0xDC, 0x5A, 0x58, 0xB1, 0xDE, 0xAB, 0xC6, 0xF4, 0x95, 0x0F, 0xEE, 0x5B, 0x98, 0xEA, 0xF8, 0x22, 0x48, 0xAF, 0xD5, 0x34, 0xB3, 0x1D, 0xA5, 0x71, 0x46, 0xA7, 0x1E, 0x9F, 0xA3, 0x2D, 0x43, 0x02, 0x88, 0xA9, 0xF8, 0xC8, 0x5A, 0x56, 0x46, 0xC0, 0x49, 0x25},
	...
	const Output Consensus::GENESIS_BLOCK_OUTPUT(
		...
		// Commitment
		(const uint8_t []){0x08, 0x9D, 0xFC, 0xAC, 0x47, 0x5C, 0x94, 0xC9, 0x78, 0x86, 0x1B, 0x3D, 0xBE, 0xF1, 0xE3, 0x7B, 0x03, 0x8C, 0xC1, 0x3F, 0x9F, 0x78, 0xDE, 0x9A, 0x4E, 0x14, 0xF3, 0x1E, 0xD3, 0x6E, 0x7A, 0x54, 0xC9},
...
// Otherwise
	...
	const uint8_t Consensus::BANNED_BLOCK_HASHES[][Crypto::BLAKE2B_HASH_LENGTH] = {
		...
		// Block 694858 (https://mwc.mw/news/rewind-of-the-chain-to-block-694-858-has-been-a-success)
...
uint64_t Consensus::getHeaderDifficulty(const Header &header) {
	...
	// Return difficulty
	return (difficulty <= UINT64_MAX) ? max(static_cast<uint64_t>(difficulty), static_cast<uint64_t>(1)) : UINT64_MAX;
}
